...
class Zend_Session extends Zend_Session_Abstract
{
     ...
     *
     * @var bitset|bool This could also be a combiniation of error codes to catch
     */
     ...
     */
    private static int $_regenerateIdState = 0;
     ...
     */
    private static int $_rememberMeSeconds = 1_209_600; // 2 weeks
    ...
    public static function sessionExists()
    {
        ...
        if ((bool) ini_get('session.use_only_cookies') == false && isset($_REQUEST[session_name()])) {
            return true;
        }
        if (self::$_unitTestEnabled) {
            return true;
     ...
     *
     * @param array|bool $options  OPTIONAL Either user supplied options, or flag indicating if start initiated automatically
     */
    ...
    private static function _processStartupMetadataGlobal()
    {
        ...
        if (isset($_SESSION['__ZF'])) {
            ...
            foreach ($_SESSION['__ZF'] as $namespace => $namespace_metadata) {
                ...
                if (isset($_SESSION['__ZF'][$namespace], $namespace_metadata['ENGH']) && $namespace_metadata['ENGH'] >= 1) {
    ...
    public static function getIterator()
    {
        ...
        return new ArrayObject([...$spaces, ...array_keys(parent::$_expiringData)]);
    }
